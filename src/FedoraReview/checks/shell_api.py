...
REVIEW_SHELL_PLUGIN_PATH = \
    '$PACKAGE_SHELL_PATH:~/.config/fedora-review/shell-plugins'

class _PluginTestParser(object):
    """
...
class shell_check(AbstractCheck):
...
def _get_plugins():
    plugins = []
    path = REVIEW_SHELL_PLUGIN_PATH
    plugindir = os.path.dirname(__file__)
    plugindir = os.path.join(plugindir, '../shell-plugins')
    plugindir = os.path.normpath(plugindir)
    path = path.replace('$PACKAGE_SHELL_PATH', plugindir)
    for d in path.split(':'):
        d = os.path.expanduser(d)
        if not os.path.exists(d):
            continue
        plugins.extend(glob(os.path.join(d, '*.sh'))
    return plugins
...
class CheckInstall(GenericCheckBase):
    '''
...
class CheckDescMacroes(GenericCheckBase):
    '''
...
class CheckFilesDuplicates(GenericCheckBase):
    '''
...
class CheckSpecDescTranlation(GenericCheckBase):
    '''
